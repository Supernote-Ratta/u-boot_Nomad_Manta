			   ...
			   fb_getvar_t var, size_t chars_left)
{
	...
	case FB_AT_UUID: {
...
#endif
		/* changed tower: add some info in fastboot. */
        case FB_PSERIAL_NO:
            s = env_get("productpsn#");
            if (s) {
                fb_add_string(response, chars_left, s, NULL);
            } else {
                ret = -1;
        ...
        case FB_MODLE_NAME:
            ...
            break;
        case FB_HARDBOARD_ID:
            s = env_get("producthwboardid#");
            if (s) {
                fb_add_string(response, chars_left, s, NULL);
            } else {
                ret = -1;
...
#endif
	{ NAME_NO_ARGS("pserialno"), FB_PSERIAL_NO},
	{ NAME_NO_ARGS("build.display.id"), FB_DISPLAY_ID},
	{ NAME_NO_ARGS("modelname"), FB_MODLE_NAME},
	{ NAME_NO_ARGS("hwboardid"), FB_HARDBOARD_ID},
};
