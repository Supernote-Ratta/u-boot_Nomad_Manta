 ...
 * registers too early after WAKEUP could cause the register to be overridden by
 * default values
 */
...
/*
 * Temp sensor might take a little time to settle even though the status bit in TMST1
...
static int tps65185_read_vcom_value(struct tps65185_priv_data *priv_data, u32 *vcom_read)
{
    int ret;
    u8 vcom_reg;
...
static int tps65185_set_vcom_value(struct udevice *dev, u32 set_value)
{
    ...
    // PROGRAMMING
    tps65185_i2c_write(priv_data, REG_VCOM2_ADJUST, vcom2_val | (1 << PAPYRUS_VCOM2_PROG));
    do {
        msleep(20);
...
static void tps65185_init_arg(struct tps65185_priv_data *priv_data)
{
    ...
    priv_data->upseq0 = SEQ_VEE(0) | SEQ_VNEG(1) | SEQ_VPOS(2) | SEQ_VDD(3);
    priv_data->upseq1 = UDLY_3ms(0) | UDLY_3ms(1) | UDLY_3ms(2) | UDLY_3ms(3);
    priv_data->dwnseq0 = SEQ_VDD(0) | SEQ_VPOS(1) | SEQ_VNEG(2) | SEQ_VEE(3);
    priv_data->dwnseq1 = DDLY_6ms(0) | DDLY_6ms(1) | DDLY_6ms(2) | DDLY_6ms(3);
...
static int tps65185_probe(struct udevice *dev)
{
    ...
    ret = gpio_request_by_name(dev, "wakeup-gpios", 0, &tps65185_priv->wake_up_gpio, GPIOD_IS_OUT);
    if (ret) {
        printf("Cannot get wakeup_pin GPIO: %d\n", ret);
        return ret;
    }
    ret = gpio_request_by_name(dev, "powerup-gpios", 0, &tps65185_priv->pwr_up_gpio, GPIOD_IS_OUT);
    if (ret) {
        printf("Cannot get pwr_up_gpio GPIO: %d\n", ret);
        dm_gpio_free(dev, &tps65185_priv->wake_up_gpio);
        return ret;
    }
    ret = gpio_request_by_name(dev, "vcomctl-gpios", 0, &tps65185_priv->vcom_gpio, GPIOD_IS_OUT);
    if (ret) {
        printf("Cannot get vcom_gpio GPIO: %d\n", ret);
        dm_gpio_free(dev, &tps65185_priv->wake_up_gpio);
        dm_gpio_free(dev, &tps65185_priv->pwr_up_gpio);
        return ret;
    }
    ret = gpio_request_by_name(dev, "poweren-gpios", 0, &tps65185_priv->pwr_en_gpio, GPIOD_IS_OUT);
    if (!ret) {
    ...
    if (ret) {
        printf("Cannot init hardware for tps65185: %d--free all gpios!\n", ret);

        // 20220605: 我们需要在uboot里面同时支持两个不同的 PMIC,所以一个初始化失败的时候，需要释放
        // 对应的 GPIO，否则会影响另外一个的初始化。比如我们把 JYT的固件烧到 BOE的板子上面。
        dm_gpio_free(dev, &tps65185_priv->wake_up_gpio);
        dm_gpio_free(dev, &tps65185_priv->pwr_up_gpio);
        dm_gpio_free(dev, &tps65185_priv->vcom_gpio);
        if(got_pwr_en_gpio) dm_gpio_free(dev, &tps65185_priv->pwr_en_gpio);
        return ret;
    }
