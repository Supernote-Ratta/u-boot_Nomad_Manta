...
static inline void tcon_write(struct ebc_tcon_priv *tcon, unsigned int reg, unsigned int value)
{
...
static inline unsigned int tcon_read(struct ebc_tcon_priv *tcon, unsigned int reg)
{
    return readl(tcon->reg + reg);
}

static inline void tcon_update_bits(struct ebc_tcon_priv *tcon, unsigned int reg, unsigned int mask, unsigned int val)
{
    unsigned int tmp;
    unsigned int *cache = tcon->regcache + (reg >> 2);
...
static int ebc_tcon_enable(struct udevice *dev, struct ebc_panel *panel)
{
    ...
    if (panel->rearrange) {
        ...
        if (3840 == panel->width && 1280 == panel->height) {
            width = panel->width;
            height = panel->height;
            vir_width = panel->width;
            vir_height = panel->height;
        } else {
            width = panel->width * 2;
    ...
    tcon_write(tcon, EBC_DSP_HTIMING0, DSP_HTOTAL(panel->lsl + panel->lbl + panel->ldl + panel->lel) | DSP_HS_END(panel->lsl));
    tcon_write(tcon, EBC_DSP_HTIMING1, DSP_HACT_END(panel->lsl + panel->lbl + panel->ldl) | DSP_HACT_ST(panel->lsl + panel->lbl - 1));
    tcon_write(tcon, EBC_DSP_VTIMING0, DSP_VTOTAL(panel->fsl + panel->fbl + panel->fdl + panel->fel) | DSP_VS_END(panel->fsl));
    tcon_write(tcon, EBC_DSP_VTIMING1, DSP_VACT_END(panel->fsl + panel->fbl + panel->fdl) | DSP_VACT_ST(panel->fsl + panel->fbl));
     ...
     */
    tcon_write(tcon, EBC_WIN_CTRL, WIN2_FIFO_ALMOST_FULL_LEVEL(496) | WIN_EN(1) | BURST_REG(7) | WIN_FIFO_ALMOST_FULL_LEVEL(240) | WIN_FMT(Y_DATA_4BPP));
     ...
     */
    tcon_write(tcon, EBC_EPD_CTRL, EINK_MODE_SWAP(1) | DSP_GD_ST(panel->lsl + panel->gdck_sta) | DSP_GD_END(panel->lsl + panel->gdck_sta + panel->lgonl) | DSP_THREE_WIN_MODE(0) | DSP_SDDW_MODE(!!panel->panel_16bit) | EPD_AUO(0) | EPD_GDRL(1) | EPD_SDSHR(1));
    tcon_write(tcon, EBC_DSP_START, DSP_SDCE_WIDTH(panel->ldl) | SW_BURST_CTRL);
    tcon_write(tcon, EBC_DSP_CTRL, DSP_SWAP_MODE(panel->panel_16bit ? 2 : 3) | DSP_VCOM_MODE(1) | DSP_SDCLK_DIV(panel->panel_16bit ? 7 : 3));
    tcon_cfg_done(tcon);
...
static int ebc_tcon_dsp_mode_set(struct udevice *dev, int update_mode, int display_mode, int three_win_mode, int eink_mode)
{
    struct ebc_tcon_priv *tcon = dev_get_priv(dev);

    tcon_update_bits(tcon, EBC_DSP_CTRL, UPDATE_MODE_MASK | DISPLAY_MODE_MASK, DSP_UPDATE_MODE(!!update_mode) | DSP_DISPLAY_MODE(!!display_mode));
    tcon_update_bits(tcon, EBC_EPD_CTRL, THREE_WIN_MODE_MASK, DSP_THREE_WIN_MODE(!!three_win_mode));
...
static int ebc_tcon_image_addr_set(struct udevice *dev, u32 pre_image_addr, u32 cur_image_addr)
{
    struct ebc_tcon_priv *tcon = dev_get_priv(dev);
...
static int ebc_tcon_lut_data_set(struct udevice *dev, unsigned int *lut_data, int frame_count, int lut_32)
{
    int i, lut_size;
    struct ebc_tcon_priv *tcon = dev_get_priv(dev);
...
static int ebc_tcon_frame_start(struct udevice *dev, int frame_total)
{
    ...
    tcon_update_bits(tcon, EBC_DSP_START, DSP_FRM_TOTAL_MASK, DSP_FRM_TOTAL(frame_total - 1));
    tcon_cfg_done(tcon);
