...
struct logo_info {
    ...
    struct grayscale_header img_hdr[15];
} __packed;
...
static int read_waveform(struct udevice *dev)
{
    ...
    flush_dcache_range((ulong)plat->lut_pbuf, ALIGN((ulong)plat->lut_pbuf + cnt, CONFIG_SYS_CACHELINE_SIZE));
...
static int get_addr_by_type(struct udevice *dev, u32 logo_type)
{
    ...
    /* changed tower: for support fastboot and loader show logo. */
    if (EINK_LOGO_FASTBOOT == logo_type || EINK_LOGO_LOADER == logo_type || EINK_LOGO_LOADER1 == logo_type) {
        logo_type = EINK_LOGO_UBOOT;
    }
    /* changed end. */

    // 20240422,hsl: kernel logo always load at offset 0.
    if(EINK_LOGO_KERNEL != logo_type && EINK_LOGO_KERNEL1 != logo_type) {
        indx = ffs(logo_type);
...
static int read_header(struct blk_desc *dev_desc, disk_partition_t *part, struct logo_info *header)
{
    int i;
...
static int read_grayscale(struct blk_desc *dev_desc, disk_partition_t *part, u32 offset, u32 size, void *buf)
{
    ...
    printf("read logo grayscale data offset=%d,size=%d,blk_offset=%d, blk_start=%d,blk_count=%d,out buf=%p\n", offset, size, blk_offset, blk_start, blk_count, buf);
...
static int image_rearrange(u8 *in_buf, u8 *out_buf, u16 w, u16 h)
{
    ...
    printf("rearrange image w: %d h: %d.\n", w, h);
    if (!in_buf || !out_buf) {
 ...
 */
static int read_needed_logo_from_partition(struct udevice *dev, u32 needed_logo, u32 *loaded_logo)
{
    int ret, i;
    disk_partition_t part;
    struct blk_desc *dev_desc;
    ...
    if ((EINK_LOGO_KERNEL != needed_logo && EINK_LOGO_KERNEL1 != needed_logo) && !logo) {
    ...
    // 20240422: hsl fix for SN 7.8/10.65.
    if (part_hdr->screen_width != panel->vir_width || part_hdr->screen_height != panel->vir_height) {
    ...
    for (i = 0; i < part_hdr->logo_count; i++) {
        ...
        printf("offset=%u, size=%d, logo_type=0x%x, w=%d, h=%d,logo=0x%x needed_logo=0x%x\n", offset, size, logo_type, img_hdr->w, img_hdr->h, logo, needed_logo);

        if ((EINK_LOGO_KERNEL == needed_logo) || (logo & logo_type)) {
            printf("loading logo_type=0x%x!\n", logo_type);
             ...
             * logo here.
             */
            if (panel->mirror && (logo_type != EINK_LOGO_KERNEL && logo_type != EINK_LOGO_KERNEL1)) {
                u32 w = panel->vir_width;
                u32 h = panel->vir_height;
                u32 mirror_buf = 0;

                mirror_buf = get_addr_by_type(dev, EINK_LOGO_UNMIRROR_TEMP_BUF);
                if (mirror_buf <= 0) {
                    printf("get mirror buffer failed\n");
                    return -EIO;
                }
                read_grayscale(dev_desc, &part, offset, size, (void *)((ulong)mirror_buf));
                image_mirror((u8 *)((ulong)mirror_buf), (u8 *)((ulong)pic_buf), w, h);
            } else if (panel->rearrange && logo_type != EINK_LOGO_KERNEL && logo_type != EINK_LOGO_KERNEL1) {
                u32 w = panel->vir_width;
                u32 h = panel->vir_height;
                u32 rearrange_buf = 0;

                /* changed tower: for a5x a62 loader. */
                if (EINK_LOGO_LOADER1 == logo_type) {
                    w = img_hdr->w;
                    h = img_hdr->h;
                }
                /* changed end. */
                rearrange_buf = get_addr_by_type(dev, EINK_LOGO_UNMIRROR_TEMP_BUF);
                if (rearrange_buf <= 0) {
                    printf("get mirror buffer failed\n");
                    return -EIO;
                }
                read_grayscale(dev_desc, &part, offset, size, (void *)((ulong)rearrange_buf));
                image_rearrange((u8 *)((ulong)rearrange_buf), (u8 *)((ulong)pic_buf), w, h);
            } else {
                read_grayscale(dev_desc, &part, offset, size, (void *)((ulong)pic_buf));
            }
            flush_dcache_range((ulong)pic_buf, ALIGN((ulong)pic_buf + size, CONFIG_SYS_CACHELINE_SIZE));
...
static int eink_display(struct udevice *dev, u32 pre_img_buf, u32 cur_img_buf, u32 lut_type, int update_mode)
{
    ...
    debug("lut_type=%d, frame num=%d, temp=%d\n", lut_type, frame_num, temperature);

    ebc_tcon_ops->wait_for_last_frame_complete(ebc_tcon_dev);
    ebc_tcon_ops->lut_data_set(ebc_tcon_dev, plat->lut_data.data, frame_num, 0);
    ebc_tcon_ops->dsp_mode_set(ebc_tcon_dev, update_mode, LUT_MODE, !THREE_WIN_MODE, !EINK_MODE);
...
static int rk_eink_display_init(void)
{
    ...
    for (uclass_first_device(UCLASS_EINK_DISPLAY, &dev); dev; uclass_next_device(&dev));
...
static int rockchip_eink_show_logo(int cur_logo_type, int update_mode)
{
    ...
    /* changed tower: not show reset and uboot logo only load kernel logo when type is uboot logo. */
    /*
     * The last_logo_type is -1 means it's first displaying
     */
    printf("show log: cur logo:0x%x  last logo:%d,panel w=%d,h=%d\n", cur_logo_type, last_logo_type, plat->width, plat->height);
    if ((last_logo_type == -1) && (cur_logo_type != EINK_LOGO_UBOOT)) {
        ...
        flush_dcache_range((ulong)logo_addr, ALIGN((ulong)logo_addr + size, CONFIG_SYS_CACHELINE_SIZE));
        eink_display(dev, logo_addr, logo_addr, WF_TYPE_RESET, EINK_UPDATE_NORMAL);
        last_logo_type = EINK_LOGO_RESET;
        last_logo_addr = logo_addr;
    } else if (-1 != last_logo_type) {
    ...
    if (cur_logo_type != EINK_LOGO_UBOOT) {
        /* changed tower: for a5x a62 loader. */
        if(cur_logo_type == EINK_LOGO_LOADER && plat->width == 3840  && plat->height == 1280) {
            cur_logo_type = EINK_LOGO_LOADER1;
        }
        ret = read_needed_logo_from_partition(dev, cur_logo_type, &loaded_logo);
        /* changed end. */
        ...
        printf("logo_addr=0x%x, logo_type=0x%x\n", logo_addr, cur_logo_type);
        if (logo_addr <= 0) {
    ...
    if (cur_logo_type == EINK_LOGO_UBOOT) {
        ...
        if (plat->mirror || plat->rearrange)
            uboot_logo_buf = get_addr_by_type(dev, EINK_LOGO_UNMIRROR_TEMP_BUF);
        } else {
        ...
        ret = read_needed_logo_from_partition(dev, EINK_LOGO_KERNEL, &loaded_logo);
        if (ret || !(loaded_logo & EINK_LOGO_KERNEL)) {
            printf("No invalid kernel logo in logo.img\n");
        } else {
            int klogo_addr = get_addr_by_type(dev, EINK_LOGO_KERNEL);
...
#else
        if(plat->width == 3840  && plat->height == 1280) {
            ret = read_needed_logo_from_partition(dev, EINK_LOGO_KERNEL1, &loaded_logo);
        } else {
            ret = read_needed_logo_from_partition(dev, EINK_LOGO_KERNEL, &loaded_logo);
        }
        if (ret) {
            printf("No invalid kernel logo in logo.img,ret=%d,panel w=%d,h=%d.\n", ret, plat->width, plat->height);
        } else {
            printf("load kernel logo sucess,panel w=%d,h=%d.\n", plat->width, plat->height);
        }
#endif
        /* changed end. */
    }
...
/* changed tower: for support show fastboot and loader logo. */
int rockchip_eink_show_fastboot_loader_logo(void)
{
    int boot_mode;
    ...
    if (BOOT_MODE_BOOTLOADER == boot_mode) {
        return rockchip_eink_show_logo(EINK_LOGO_FASTBOOT, EINK_UPDATE_DIFF);
    } else if (BOOT_MODE_LOADER == boot_mode) {
        return rockchip_eink_show_logo(EINK_LOGO_LOADER, EINK_UPDATE_DIFF);
    }
#endif
    return 0;
}
...
static int rockchip_eink_display_probe(struct udevice *dev)
{
    ...
    ret = uclass_get_device_by_phandle(UCLASS_EBC, dev, "ebc_tcon", &priv->ebc_tcon_dev);
    if (ret) {
    ...
    for (i = 0; i < size; i++) {
        ...
        ret = uclass_get_device_by_phandle_id(UCLASS_I2C_GENERIC, phandle, &priv->ebc_pwr_dev);
        if (!ret) {
...
static int rockchip_eink_display_ofdata_to_platdata(struct udevice *dev)
{
    ...
    disp_mem = of_parse_phandle(ofnode_to_np(dev_ofnode(dev)), "memory-region", 0);
    if (!disp_mem) {
    ...
    debug("display mem=0x%x, size=%x\n", plat->disp_pbuf, plat->disp_pbuf_size);
    waveform_mem = of_parse_phandle(ofnode_to_np(dev_ofnode(dev)), "waveform-region", 0);
    if (!waveform_mem) {
        printf("Cannot get waveform-region from dts\n");
        return -ENODEV;
    }
    tmp_addr = ofnode_get_addr_size(np_to_ofnode(waveform_mem), "reg", &size);
    if (tmp_addr == FDT_ADDR_T_NONE) {
