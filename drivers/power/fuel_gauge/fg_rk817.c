...
static void rk817_charge_set_term_current(struct rk817_battery_device *battery,
						  int chrg_current)
{
	int value;
	u8 current_adc;

	value = CURRENT_TO_ADC(chrg_current, battery->res_div);

	value &= (0xff << 5);
	current_adc = value >> 5;

	rk817_bat_write(battery, CHRG_TERM_DIG, current_adc);

	value = rk817_bat_read(battery, CHRG_TERM_ANA_DIG);
	value |= 0x4;
	
	rk817_bat_write(battery, CHRG_TERM_ANA_DIG, value);
}
...
#ifdef CONFIG_DM_POWER_DELIVERY
static int rk817_find_cur_value(u32 value)
{
	switch(value) {
	case 450000:
		return ILIM_450MA;
...
static int rk817_get_pd_output_val(struct rk817_battery_device *battery,
				     int *vol, int *cur)
{
	...
	ret = power_delivery_get_data(battery->pd, &pd_data);
	if (ret)
		return ret;
	if(pd_data.online)
		battery->cc_connected = true;
...
static void rk817_bat_set_charge_voltage(struct rk817_battery_device *battery,int voltage)
{
	int voltage_toset, value;

	value = rk817_bat_read(battery, PMIC_CHRG_OUT);

	printf("=====rk817_bat_set_charge_voltage max_chrg_voltage: %d value=0x%x\n",voltage,value);
	if (voltage < 4100 || voltage > 4500) {
		voltage = 4200;
    }
    voltage_toset = (voltage - 4100) / 50;
	rk817_bat_write(battery, PMIC_CHRG_OUT, (value & 0x8f) | ((CHRG_VOL_4100MV + voltage_toset)<<4));
}

static void rk817_bat_set_charge_current(struct rk817_battery_device *battery,int current)
{
	int value;

	value = rk817_bat_read(battery, PMIC_CHRG_OUT);
	printf("=====rk817_bat_set_charge_current : %d val:0x%x\n",current,value);
	rk817_bat_write(battery, PMIC_CHRG_OUT, (value & 0xf8) | current);
}
...
static int rk817_bat_get_charger_type(struct rk817_battery_device *battery)
{
	...
	chrg_state = rk817_bat_read(battery, 0xeb);
	if ((chrg_type != NO_CHARGER && battery->rsoc / 100 >= 999) ||
	((chrg_state & 0x70) == CHARGE_FINISH))
		chrg_type = CHARGE_FINISH;
	printf("rk817_bat_get_charger_type:type=%d rsoc:%d\n",chrg_type,battery->rsoc);

	return chrg_type;
}
static void rk817_bat_enable_battery_charge(struct rk817_battery_device *battery)
{
	rk817_bat_write(battery, 0xe9, 0xfA);
}

static void rk817_bat_disable_battery_charge(struct rk817_battery_device *battery)
{
	rk817_bat_write(battery, 0xe9, 0x05);
}
...
static int rk817_bat_get_temp(struct rk817_battery_device *battery)
{
	...
	temp_value1 |= rk817_bat_read(battery, 0xe9);
	...
	printf("rk817_bat_get_temp:adc=%d adcv=%d templimit:0x%2x size:%d\n",temp_value,adc_to_vol,temp_value1,battery->temp_size);
	if((battery->temp_size==0)||(battery->tempn_size==0)){
		return 250;
	}
	battery->adc_count++;
	...
	printf("rk817_bat_get_temp:adc_avg=%d\n",adc_avg);
	if(adc_avg > battery->tempadc_table[0]){  //temperatrue < 0
		for(i=0;i<battery->tempn_size;i++){
			if(adc_avg < battery->tempnadc_table[i]){
				//return -i*10;
                return -i*10;
			}
		}
	}else{
		for(i=1;i<battery->temp_size;i++){ //temperatrue > 0
			if(adc_avg > battery->tempadc_table[i]){
				//return (i-1)*10;
                return (i-1)*10;
		}
	}
...
// 10-45 正常充电
	...
	printf("=====rk817_bat_temperature_chrg temp:%d temperature_disable_charge:%d temperature_charge_reset:%d current:%d l:%d\n",
		temp,temperature_disable_charge,temperature_charge_reset,battery->current_avg,battery->dsoc);
	...
	if ((temp <= 0)||(temp >= 50)){
		if(temperature_disable_charge == 0) {
			...
			printf("rk817_bat_temperature_chrg:rk817_bat_disable_charge \n");
		}
	...
	if(temperature_charge_reset){
		...
		printf("rk817_bat_temperature_chrg:vol_lower_3:%d vol:%d \n",vol_lower_3,rk817_bat_get_battery_voltage(battery));
		if(vol_lower_3){
			rk817_bat_set_charge_voltage(battery,4400);
			rk817_bat_set_charge_current(battery,CHRG_CUR_500MA);
			printf("rk817_bat_temperature_chrg:vol_lower_3:500mA,4.4V \n");
			temperature_charge_reset = 0;
			rk817_bat_enable_battery_charge(battery);
			return 0;
		}
		if( temp >= 45 ){
			rk817_bat_set_charge_voltage(battery,4200);
			rk817_bat_set_charge_current(battery,CHRG_CUR_1500MA);
			printf("=====rk817_bat_temperature_chrg 1.5A,4.2V\n");
		}else if( temp >= 15 ){
			rk817_bat_set_charge_voltage(battery,4400);
			rk817_bat_set_charge_current(battery,CHRG_CUR_2000MA);
			printf("=====rk817_bat_temperature_chrg 2A,4.4V\n");
		}else{
			rk817_bat_set_charge_voltage(battery,4400);
			rk817_bat_set_charge_current(battery,CHRG_CUR_500MA);
			printf("=====rk817_bat_temperature_chrg 0.5A,4.4V\n");
		}
	}else{
		if ((temp > 1) && (temp < 49)) {
			temperature_disable_charge = 0;
		...
		if(vol_lower_3){
			...
			rk817_bat_set_charge_voltage(battery,4400);
			rk817_bat_set_charge_current(battery,CHRG_CUR_500MA);
			if(rk817_bat_get_battery_voltage(battery) < 3000){
				printf("rk817_bat_temperature_chrg:vol_lower_3:500mA,4.4V \n");
			}else{
				if(battery->dsoc>500){
					vol_lower_3 = false;
					printf("rk817_bat_temperature_chrg:vol > 3,battery->dsoc>800 500mA,4.4V \n");
				}
			}
			temperature_charge_reset = 0;
			rk817_bat_enable_battery_charge(battery);
				      ...
				      int charger)
{
	...
	if (old_charger != charger) {
		...
		if (charger == NO_CHARGER) {
			...
			printf("DC OR AC CHARGE\n");
			power_type = rk817_bat_dwc_otg_check_dpdm(battery);
			printf("rk817_bat_charger_setting power supply:%d\n",power_type);
#ifdef CONFIG_DM_POWER_DELIVERY
			if(battery->cc_connected) {
				if(battery->current_pd != 0) {
					printf("PD CHARGE\n");
					rk817_bat_set_input_current(battery, battery->current_pd);
				}
				else {
					if(power_type == POWER_SUPPLY_TYPE_USB_FLOATING){
						printf("CC CONNECTED, FLOATING CHARGE 450MA\n");
						rk817_bat_set_input_current(battery, ILIM_450MA);
					}else{
						printf("CC CONNECTED, DCP CHARGE 2000MA\n");
						rk817_bat_set_input_current(battery, ILIM_2000MA);
					}
				}
			}
			else {
				printf("CC DISCONNECTED, CHARGE 450MA\n");
				rk817_bat_set_input_current(battery, ILIM_450MA);
			}
#else
	...
	temper = rk817_bat_get_temp(battery);
	rk817_bat_temperature_chrg(battery,temper/10);
}
...
static void rk817_bat_finish_chrg(struct rk817_battery_device *battery)
{
	...
	printf("rk817_bat_finish_chrg :finish_chrg_base=%ld--%ld tgt_sec=%d--%d\n",battery->finish_chrg_base,get_timer(battery->finish_chrg_base),tgt_sec,SECONDS(tgt_sec));
...
static int rk817_bat_get_battery(struct rk817_battery_device *battery)
{
	int  temp_value = 0;
	...
	if(temp_value>65530){
		printf("=====rk817_bat_temperature_chrg no battery!!!\n");
		return 0;
	}
	return 1;
}

static int rk817_bat_get_poweron_source(struct udevice *dev)
{
	struct rk817_battery_device *battery = dev_get_priv(dev);
	int  temp_value = 0;
	temp_value = rk817_bat_read(battery, PMIC_ON_SOURCE);
	printf("=====rk817_bat_get_power_on_source:%d\n",temp_value);
	return temp_value;
}
...
static int rk817_is_bat_exist(struct rk817_battery_device *battery)
{
	...
	if (rk8xx->variant == RK817_ID)
		return rk817_bat_get_battery(battery);//(rk817_bat_read(battery, PMIC_CHRG_STS) & 0x80) ? 1 : 0;//tanlq mod 220926
...
static struct dm_fuel_gauge_ops fg_ops = {
	...
	.get_poweron_source = rk817_bat_get_poweron_source,
};
...
/////////////////////////////////
	prop = dev_read_prop(dev,  "tempadc_table", &len);
	if (!prop) {
		printf("can't find tempadc_table prop\n");
		battery->temp_size = 0;
	}else{
		battery->tempadc_table = calloc(len, 1);
		battery->temp_size = len / 4;
		if (dev_read_u32_array(dev, "tempadc_table",
		       battery->tempadc_table, battery->temp_size)) {
			printf("can't read tempadc_table\n");
			free(battery->tempadc_table);
			battery->temp_size = 0;
		}else{
			for(i=0;i<battery->temp_size;i++)
				printf("tempadc_table[%d]=%d\n",i,battery->tempadc_table[i]);
		}
	}
	if (battery->temp_size <= 0) {
		printf("invalid tempadc table\n");
		//return -EINVAL;
	}

	prop = dev_read_prop(dev, "tempnadc_table", &len);
	if (!prop) {
		printf("can't find tempnadc_table prop\n");
		battery->tempn_size = 0;
	}else{
		battery->tempnadc_table = calloc(len, 1);
		battery->tempn_size = len / 4;
		if (dev_read_u32_array(dev, "tempnadc_table",
			   battery->tempnadc_table, battery->tempn_size)) {
			printf("can't read tempnadc_table\n");
			free(battery->tempnadc_table);
			battery->tempn_size = 0;
		}else{
			for(i=0;i<battery->tempn_size;i++)
				printf("tempnadc_table[%d]=%d\n",i,battery->tempnadc_table[i]);
		}
	}
	if (battery->tempn_size <= 0) {
		printf("invalid tempadc tempnadc_table\n");
		//return -EINVAL;
	}
...
///////////////////////////////////////////////
	...
	printf("=====battery->qmax =%d\n",battery->qmax);

	battery->max_chrg_voltage = dev_read_u32_default(dev, "max_chrg_voltage", -1);
	if (battery->max_chrg_voltage < 0) {
		printf("can't read max_chrg_voltage,set to 4200\n");
		//return -EINVAL;
	}
	printf("=====battery->max_chrg_voltage =%d\n",battery->max_chrg_voltage);
	...
	DBG("battery->max_chrg_voltage: %d\n", battery->max_chrg_voltage);
...
static int rk817_fg_init(struct rk817_battery_device *battery)
{
	...
	rk817_bat_set_initialized_flag(battery);
	rk817_bat_set_charge_voltage(battery,battery->max_chrg_voltage);
	rk817_charge_set_term_current(battery, CHARGE_FINISH_CURRENT);
...
#ifdef CONFIG_DM_POWER_DELIVERY
	battery->current_pd = 0;
	battery->cc_connected = false;
	if (!rk817_get_pd_output_val(battery, &pd_inputvol,
				       &pd_inputcurrent)) {
		printf("%s pd charge input vol:%duv current:%dua\n",
		       __func__, pd_inputvol, pd_inputcurrent);

		if (pd_inputcurrent != 0) {
			battery->current_pd = rk817_find_cur_value(pd_inputcurrent);
		}
	}
#endif
...
#ifdef CONFIG_DM_POWER_DELIVERY
	ret = uclass_get_device(UCLASS_PD, 0, &battery->pd);
	if (ret) {
		if (ret == -ENODEV)
