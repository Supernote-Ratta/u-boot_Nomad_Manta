...
#define LED_CHARGING_NAME			"led_r"
#define LED_CHARGING_FULL_NAME			"led_g"
 ...
 */
	...
	/* changed tower: support android reboot charge command. */
...
#endif
	/* changed tanlq: for Push/pull charge quickly, it will start kernel */
	charging = fg_charger_get_chrg_online(dev);
	poweron_source = fuel_gauge_get_poweron_source(fg);
	printf("fuel_gauge_get_poweron_source:0x%x\n",poweron_source);
	if (charging <= 0) {
		printf("Exit charge: due to charger offline...\n");
		/* changed tower: will shutdown when pull out charger before poweroff fast. */
		if(poweron_source == 0x40 || strstr(preboot, "charge")){
			printf("poweron_source=0x%x,Charge offline,Shutdown...\n",poweron_source);
			pmic_shutdown(pmic);
		}
		return 0;
	}
	...
	if (charging <= 0) {
		printf("Exit charge: due to charger offline...\n");
		/* changed tanlq: add for Push/pull charge quickly, it will start kernel */
		if(poweron_source == 0x40){
			printf("poweron_source=0x%x,Charge offline,Shutdown...\n",poweron_source);
			pmic_shutdown(pmic);
		}
		/* changed end. */
