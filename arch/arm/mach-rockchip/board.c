...
/* changed tower: generate serial when no write it in nv. */
static int rockchip_generate_serialno(void)
{
...
static int rockchip_set_serialno(void)
{
    ...
    memset(serialno_str, 0, VENDOR_SN_MAX);
#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	/* changed tower: read serial no 8 byte. */
#ifdef CONFIG_EINK_PROJECT_BOE
	ret = vendor_storage_read(SN_ID, serialno_str, 8);
#else
	...
	if (ret > 0) {
		...
		for (; i > 0; i--) {
			if ((serialno_str[i - 1] >= 'a' && serialno_str[i - 1] <= 'z') ||
				(serialno_str[i - 1] >= 'A' && serialno_str[i - 1] <= 'Z') ||
				(serialno_str[i - 1] >= '0' && serialno_str[i - 1] <= '9'))
				break;
		}

        if (0 > (i - 1)) {
            return rockchip_generate_serialno();
        }
...
/* changed tower: add product info to set cmdline for set property in android. */
#define WIFI_BT_LEN         0x11
#define COUNTRY_CODE_LEN    0x4
#define PRODUCT_REGION_LEN  0x3
#define COUNTRY_LEN         0x2
#define COUNTRY_MASK        0xFFFF

static int rockchip_set_countrycode(void)
{
	const char *vaild_country[] = {
		"GB", "TR", "PL", "RO", "BG", "HU", "GR", "HR", "SI", "CY", "RS", "CZ", "SK",
		"DE", "IT", "AT", "FR", "NL", "BE", "SE", "FI", "LV", "LT", "EE", "DK", "NO",
		"IS", "CH", "ES", "PT", "AE", "SA", "EG", "ZA", "PK", "LB", "TN", "CN", "UA",
		"PE", "CO", "MX", "CL", "AU", "NZ"
	};
	const char *default_language[] = {
		"en-GB", "tr-TR", "pl-PL", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US",
		"en-US", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US", "en-US",
		"en-US", "de-CH", "en-US", "en-US", "ar-EG", "ar-EG", "ar-EG", "en-US", "en-US", "en-US", "fr-FR", "zh-Hans-CN", "uk-UA",
		"es-US", "es-US", "es-US", "es-US", "en-US", "en-US"
	};
	char country_code[COUNTRY_CODE_LEN + 1], country[COUNTRY_LEN + 1] = {0};
	int ret = 0, i = 0;

	/* Read country code from vendor storage part */
	memset(country_code, 0, COUNTRY_CODE_LEN + 1);

#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	ret = vendor_storage_read(COUNTRY_CODE_ID, country_code, COUNTRY_CODE_LEN);
	if (ret > 0) {
		i = strlen(country_code);
		for (; i > 0; i--) {
			if ((country_code[i] >= 'a' && country_code[i] <= 'z') || (country_code[i] >= 'A' && country_code[i] <= 'Z') || (country_code[i] >= '0' && country_code[i] <= '9')) {
				break;
			}
		}

		country_code[i + 1] = 0x0;
		country[0] = country_code[0];
		country[1] = country_code[1];
		country[2] = 0x0;
		printf("product countrycode: %s\n", country);
		env_set("productcountry#", country);
		for (i = 0; i < ARRAY_SIZE(vaild_country); i++) {
			if (!strcmp(vaild_country[i], country)) {
				printf("product deflanguage:%s\n", default_language[i]);
				env_set("productlanguage#", default_language[i]);
			}
		}
	} else {
#endif
		printf("not found country code ...\n");
#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	}
#endif
...
static int rockchip_set_productregion(void)
{
	char product_region[PRODUCT_REGION_LEN + 1];
	int ret = 0, i = 0;

	/* Read country code from vendor storage part */
	memset(product_region, 0, PRODUCT_REGION_LEN + 1);

#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	ret = vendor_storage_read(PRODUCT_REGION, product_region, PRODUCT_REGION_LEN);
	if (ret > 0) {
		i = strlen(product_region);
		for (; i > 0; i--) {
			if ((product_region[i] >= 'A' && product_region[i] <= 'Z')) {
				break;
			}
		}

		product_region[i + 1] = 0x0;
		printf("product region: %s\n", product_region);
		env_set("productregion#", product_region);
	} else {
#endif
		printf("not found product region ...\n");
#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	}
#endif
...
static int rockchip_set_productpsn(void)
{
	char product_psn[VENDOR_SN_MAX];
	int ret = 0, i = 0;

	/* Read psn from vendor storage part */
	memset(product_psn, 0, VENDOR_SN_MAX);

#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	ret = vendor_storage_read(PRODUCT_PSN, product_psn, VENDOR_SN_MAX - 1);
	if (ret > 0) {
		i = strlen(product_psn);
		for (; i > 0; i--) {
			if ((product_psn[i] >= 'a' && product_psn[i] <= 'z') || (product_psn[i] >= 'A' && product_psn[i] <= 'Z') || (product_psn[i] >= '0' && product_psn[i] <= '9')) {
				break;
			}
		}

		product_psn[i + 1] = 0x0;
		printf("product psn: %s\n", product_psn);
		env_set("productpsn#", product_psn);
	} else {
#endif
		printf("not found product psn ...\n");
#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	}
#endif
...
static int rockchip_set_productmodlename(void)
{
	char product_modlename[VENDOR_SN_MAX];
	int ret = 0, i = 0;

	/* Read country code from vendor storage part */
	memset(product_modlename, 0, VENDOR_SN_MAX);

#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	ret = vendor_storage_read(MODLENAME_ID, product_modlename, VENDOR_SN_MAX - 1);
	if (ret > 0) {
		i = strlen(product_modlename);
		product_modlename[i + 1] = 0x0;
		printf("product modle name: %s\n", product_modlename);
		env_set("productmodlename#", product_modlename);
	} else {
#endif
		printf("not found product modle name ...\n");
#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	}
#endif
...
static int rockchip_set_displayid(void)
{
	char product_displayid[VENDOR_SN_MAX];
	int ret = 0, i = 0;

	/* Read country code from vendor storage part */
	memset(product_displayid, 0, VENDOR_SN_MAX);

#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	ret = vendor_storage_read(DISPLAY_ID, product_displayid, VENDOR_SN_MAX - 1);
	if (ret > 0) {
		i = strlen(product_displayid);
		product_displayid[i + 1] = 0x0;
		printf("product display id: %s\n", product_displayid);
		env_set("produceversion#", product_displayid);
	} else {
#endif
		printf("not found product display id...\n");
#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	}
#endif
...
static int rockchip_set_hwboardid(void)
{
	char product_boardid[VENDOR_SN_MAX];
	int ret = 0, i = 0;

	/* Read country code from vendor storage part */
	memset(product_boardid, 0, VENDOR_SN_MAX);

#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	ret = vendor_storage_read(HWBOARD_ID, product_boardid, VENDOR_SN_MAX - 1);
	if (ret > 0) {
		i = strlen(product_boardid);
		product_boardid[i + 1] = 0x0;
		printf("product board id: %s\n", product_boardid);
		env_set("producthwboardid#", product_boardid);
	} else {
#endif
		printf("not found product board id ...\n");
#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	}
#endif
...
static int rockchip_set_wifiaddr(void)
{
#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	char buf[WIFI_BT_LEN + 1], mac[17];
	u8 wifiaddr[WIFI_BT_LEN + 1] = { 0 };
	int ret;

	ret = vendor_storage_read(WIFI_MAC_ID, wifiaddr, WIFI_BT_LEN);
	if (ret > 0) {
		if (is_valid_ethaddr(wifiaddr)) {
			sprintf(buf, "%s", wifiaddr);
			memcpy(mac, "productwifiaddr#", sizeof("productwifiaddr#"));
			printf("product wifi addr: %s\n", buf);
			env_set(mac, buf);
		}
	}
#endif
...
static int rockchip_set_btaddr(void)
{
#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	char buf[WIFI_BT_LEN + 1], mac[16];
	u8 btaddr[WIFI_BT_LEN + 1] = { 0 };
	int ret;

	ret = vendor_storage_read(BT_MAC_ID, btaddr, WIFI_BT_LEN);
	if (ret > 0) {
		sprintf(buf, "%s", btaddr);
		memcpy(mac, "productbtaddr#", sizeof("productbtaddr#"));
		printf("product bt addr: %s\n", buf);
		env_set(mac, buf);
	}
#endif
...
int board_late_init(void)
{
	...
	/* changed tower: add country code and product region to set cmdline for set property in android. */
	rockchip_set_countrycode();
	rockchip_set_productregion();
	rockchip_set_productpsn();
	rockchip_set_productmodlename();
	rockchip_set_displayid();
	rockchip_set_hwboardid();
	rockchip_set_wifiaddr();
	rockchip_set_btaddr();
	/* changed end. */
	setup_download_mode();
	...
	rockchip_show_logo();
#endif
	/* changed tower: support fastboot/loader logo show. */
#ifdef CONFIG_ROCKCHIP_EINK_DISPLAY
	rockchip_eink_show_uboot_logo();
	rockchip_eink_show_fastboot_loader_logo();
#endif
	/* changed end. */
	env_fixup();
	soc_clk_dump();
