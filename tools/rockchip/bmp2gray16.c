...
uint32_t screen_w;
uint32_t screen_h;
static const char version[5] = "1.01";
...
static inline int size_of_kernel_image(void)
{
    return ALIGN((screen_w * screen_h), RK_BLK_SIZE);
}

int get_logo_resolution(char *in_path, uint32_t *logo_width, uint32_t *logo_height)
{
 ...
 */
int convert_bmp_idx_to_gray_idx(int idx, uint32_t w, uint32_t h, int is_kernel)
{
    int row = h - (idx / w) - 1;
...
int convert_one_image(char *in_path, void *out_buf, uint32_t offset, struct grayscale_header *gr_hdr, int type)
{
    ...
    if (EINK_LOGO_KERNEL == type) {
        gr_hdr->data_size = screen_w * screen_h;
    } else {
    ...
    */
    ...
    switch (bmp_hdr->bit_count) {
        case 16: {
            ...
            if (EINK_LOGO_KERNEL == type || EINK_LOGO_KERNEL1 == type) {
                for (i = 0; i < gr_hdr->w * gr_hdr->h; i++) {
                    struct pixel_u16 *pix = &color_u16[i];
                    int j = convert_bmp_idx_to_gray_idx(i, gr_hdr->w, gr_hdr->h, 1);
                    /*
                     * the rgb value of pixel_u16 is 8 bits
                     */
                ...
                for (i = 0; i < screen_w * screen_h / 2; i++) {
                    ...
                    int j = convert_bmp_idx_to_gray_idx(2 * i, screen_w, screen_h, 0);
                    /*
        ...
        case 24: {
            ...
            if (EINK_LOGO_KERNEL == type || EINK_LOGO_KERNEL1 == type) {
                for (i = 0; i < gr_hdr->w * gr_hdr->h; i++) {
                     ...
                     */
                    uint32_t gray_px = ((pix->red * 19595 + pix->green * 38469 + pix->blue * 7472) >> 16) >> 4;
                    gr16_data[j] = gray_px;
                }
            } else {
                for (i = 0; i < screen_w * screen_h / 2; i++) {
...
void *init_grayscale_logo_buf(int logo_count, uint32_t size_one_image, int w, int h,
    int has_kernel, int has_kernel1)
{
    int size;
    void *out_buf;
    ...
    size = size_of_header();
    fprintf(stderr, "size of header in logo.img is %d one image size: %u\n", size, size_one_image);
    //every pixel of the grayscale image is 4 bits
    size += logo_count * size_one_image;
    if (has_kernel) {
        size += size_of_kernel_image() - size_one_image;
    }
...
static int save_buf_to_file(const void *buf, int size, const char *fp)
{
    int fd = open(fp, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0);
    if (fd < 0) {
        fprintf(stderr, "create/open newfw file:%s Failed:%s\n", fp, strerror(errno));
        return -1;
    }
    ...
    close(fd);
    if (rc != size) {
        fprintf(stderr, "write newfw file:%s error,size=%d,fsize=%d\n", fp, size, rc);
        return -2;
    }

    fprintf(stderr, "save new fw file %s OK,size = %d\n", fp, rc);
    return 0;
}


// 20240422: add kernel2-logo for SN compitable 7.8/10.65.
int main(int argc, char *argv[])
{
    ...
    printf("\nbmp2gray16: ht fix for SN,version=20240423-4.\n");
    argc--, argv++;
    while (argc > 0 && argv[0][0] == '-') {
        ...
        if (!strcmp("-h", arg)) {
            ...
            for (i = 0; i < 7; i++) {
                ...
                len = strlen(argv[0]);
                if (len > 256) {
                    fprintf(stderr, "input charging logo path %s is too long.\n", argv[0]);
                    return -1;
                }
            ...
            logo_count++;
            argc--, argv++;
        } else if (!strcmp("--kernel1-logo", arg)) {
            int len = strlen(argv[0]);

            if (len > 256) {
                printf("Kernel1 logo path %s is too long\n", argv[0]);
                return -1;
            }
            memcpy(in_img_info[logo_count].path, argv[0], len);
            in_img_info[logo_count].logo_type = EINK_LOGO_KERNEL1;
            logo_count++;
            argc--, argv++;
        } else if (!strcmp("--fastboot-logo", arg)) {
            int len = strlen(argv[0]);
            ...
            memcpy(in_img_info[logo_count].path, argv[0], len);
            in_img_info[logo_count].logo_type = EINK_LOGO_FASTBOOT;
            logo_count++;
            argc--, argv++;
        } else if (!strcmp("--loader-logo", arg)) {
            int len = strlen(argv[0]);
            ...
            memcpy(in_img_info[logo_count].path, argv[0], len);
            in_img_info[logo_count].logo_type = EINK_LOGO_LOADER;
            logo_count++;
            argc--, argv++;
        } else if (!strcmp("--loader1-logo", arg)) {
            int len = strlen(argv[0]);

            if (len > 256) {
                printf("loader1 logo path %s is too long\n", argv[0]);
                return -1;
            }
            memcpy(in_img_info[logo_count].path, argv[0], len);
            in_img_info[logo_count].logo_type = EINK_LOGO_LOADER1;
            logo_count++;
            argc--, argv++;
    ...
    */
    ...
    struct logo_info tmp_hdr;
    // 20240423,get total size for alloc buffer.
    ...
    for (i = 0; i < logo_count; i++) {
        ...
        struct grayscale_header *tmp_header = &tmp_hdr.img_hdr[i];
        int wx = 0, hx = 0;
        if (EINK_LOGO_KERNEL == in_img_info[i].logo_type) {
            ret = get_logo_resolution(in_img_info[i].path, &wx, &hx);
            if (ret < 0) {
            ...
            tmp_header->data_size = ALIGN((wx * hx), RK_BLK_SIZE);  // size_of_kernel_image();
        } else if (EINK_LOGO_KERNEL1 == in_img_info[i].logo_type) {
            ret = get_logo_resolution(in_img_info[i].path, &wx, &hx);
            tmp_header->data_size = ALIGN((wx * hx), RK_BLK_SIZE);  // size_of_kernel_image();
        } else {
            ret = get_logo_resolution(in_img_info[i].path, &wx, &hx);
            tmp_header->data_size = ALIGN((wx * hx) >> 1, RK_BLK_SIZE);  // size_of_one_image();
        }

        tmp_header->data_offset = offset;
        total_size += tmp_header->data_size;
        fprintf(stderr, "get %s bmp w=%d,h=%d,data_size=%d,offset=%d,total_size=%d.\n", in_img_info[i].path, wx, hx, tmp_header->data_size, offset, total_size);
        tmp_header->w = wx;
        tmp_header->h = hx;
        offset += tmp_header->data_size;
    }

    fprintf(stderr, "The total_size=%d\n", total_size);
    out_buf = calloc(1, total_size);
    // out_buf = init_grayscale_logo_buf(logo_count, one_img_size, screen_w, screen_h, has_kernel_log, has_kernel1_log);
    if (!out_buf) {
    ...
    for (i = 0; i < logo_count; i++) {
        ...
        offset = tmp_header->data_offset;
        img_buf = out_buf + offset;
        logo_hdr->img_hdr[i].data_offset = tmp_header->data_offset;
        logo_hdr->img_hdr[i].data_size = tmp_header->data_size;
        logo_hdr->img_hdr[i].logo_type = type;
        logo_hdr->img_hdr[i].w = tmp_header->w;
        logo_hdr->img_hdr[i].h = tmp_header->h;
        screen_w = tmp_header->w;
        screen_h = tmp_header->h;
        printf("image[%d] start addr=0x%p hdr size: %d offset: %d type: %d data size: 0x%x\n", i, img_buf, hdr_size, offset, type, tmp_header->data_size);
        ret = convert_one_image(in_path, img_buf, offset, &logo_hdr->img_hdr[i], type);
        if (ret < 0) {
    ...
    if (ret == 0) {
        ...
        /*
        total_size = hdr_size + (i - 1) * one_img_size + screen_h * screen_w / 2;
        if (has_kernel_log) {
            total_size += size_of_kernel_image() - one_img_size;
        }
        ...
        */
        save_buf_to_file(out_buf, total_size, out_path);
    }
