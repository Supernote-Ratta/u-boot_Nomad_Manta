				      ...
				      const char *extra_args)
{
    ...
    /* changed tower: add country code to set cmdline for set property in android. */
    char *productcountry, *allocated_productcountry = NULL;
	char *productregion, *allocated_productregion = NULL;
    char *productwifimac, *allocated_productwifimac = NULL;
    char *productbtmac, *allocated_productbtmac = NULL;
    char *productlanguage, *allocated_productlanguage = NULL;
    /* changed end. */
    ...
    productcountry = env_get("productcountry#");
    if (productcountry) {
        allocated_productcountry = malloc(strlen(ANDROID_ARG_PRODUCTCOUNTRYCODE) + strlen(productcountry) + 1);
        memset(allocated_productcountry, 0, strlen(ANDROID_ARG_PRODUCTCOUNTRYCODE) + strlen(productcountry) + 1);
        strcpy(allocated_productcountry, ANDROID_ARG_PRODUCTCOUNTRYCODE);
        strcat(allocated_productcountry, productcountry);
        *(current_chunk++) = allocated_productcountry;
    }

    productlanguage = env_get("productlanguage#");
    if (productlanguage) {
        allocated_productlanguage = malloc(strlen(ANDROID_ARG_PRODUCTLANGUAGE) + strlen(productlanguage) + 1);
        memset(allocated_productlanguage, 0, strlen(ANDROID_ARG_PRODUCTLANGUAGE) + strlen(productlanguage) + 1);
        strcpy(allocated_productlanguage, ANDROID_ARG_PRODUCTLANGUAGE);
        strcat(allocated_productlanguage, productlanguage);
        *(current_chunk++) = allocated_productlanguage;
    }

	productregion = env_get("productregion#");
    if (productregion) {
        allocated_productregion = malloc(strlen(ANDROID_ARG_PRODUCTREGION) + strlen(productregion) + 1);
        memset(allocated_productregion, 0, strlen(ANDROID_ARG_PRODUCTREGION) + strlen(productregion) + 1);
        strcpy(allocated_productregion, ANDROID_ARG_PRODUCTREGION);
        strcat(allocated_productregion, productregion);
        *(current_chunk++) = allocated_productregion;
    }

    productwifimac = env_get("productwifiaddr#");
    if (productwifimac) {
        allocated_productwifimac = malloc(strlen(ANDROID_ARG_PRODUCTWIFIMAC) + strlen(productwifimac) + 1);
        memset(allocated_productwifimac, 0, strlen(ANDROID_ARG_PRODUCTWIFIMAC) + strlen(productwifimac) + 1);
        strcpy(allocated_productwifimac, ANDROID_ARG_PRODUCTWIFIMAC);
        strcat(allocated_productwifimac, productwifimac);
        *(current_chunk++) = allocated_productwifimac;
    }

    productbtmac = env_get("productbtaddr#");
    if (productbtmac) {
        allocated_productbtmac = malloc(strlen(ANDROID_ARG_PRODUCTBTMAC) + strlen(productbtmac) + 1);
        memset(allocated_productbtmac, 0, strlen(ANDROID_ARG_PRODUCTBTMAC) + strlen(productbtmac) + 1);
        strcpy(allocated_productbtmac, ANDROID_ARG_PRODUCTBTMAC);
        strcat(allocated_productbtmac, productbtmac);
        *(current_chunk++) = allocated_productbtmac;
    }
	/* changed end. */
